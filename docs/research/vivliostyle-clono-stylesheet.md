...
- WindowsとLinuxにおけるWebPubおよびPDF生成
- npmパッケージ方式を再検討する場合の、公開済みclono、ネットワーク、キャッシュ、導入時間および依存関係の重複

生成済み原稿ツリーへの基盤CSSのコピーは、実装後のCLI統合テストをCIでUbuntu、macOSおよびWindows上から実行して確認している。VivliostyleによるWebPubおよびPDF生成の検証環境は、引き続きmacOSだけである。
