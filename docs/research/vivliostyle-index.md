...
- 同じ項目が同じページに複数回ある場合、ページ番号を一つにまとめる
...
同一ページ重複の除去方法が未決定であるため、索引機能全体の責務判断は確定しない。
...
- 組版後のページ番号に基づいて、同一ページの重複を除去する
- 同一ページ重複を除去した後も、ページ番号からどの索引マーカーへリンクするかを決定する
