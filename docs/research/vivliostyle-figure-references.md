...
図カウンターには`figure`という固有の名前を使用する。表とコードリストのカウンターは別名にできるが、三種類を同じ文書へ配置した結合テストはまだ実施していない。
...
- 番号付き画像を、IDと用途別のclassを持つ`figure`、代替テキストを持つ`img`、キャプションを持つ`figcaption`へ変換する
