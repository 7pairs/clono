...
- 空の用語または許可しないインライン要素を持つ用語を、変換前に位置付きで診断できるか確認する
...
用語内のリンクは`link`ノードとして解析された。用語の子ノードを`text`と`inlineCode`の許可リストで検証することで、リンクを許可しない内容として位置付きで診断できた。空の用語も同様に診断でき、いずれの場合もAST変換を実行せず、部分的な出力Markdownを生成しなかった。
...
ページ末尾の残り領域は、固定したページ寸法、行高およびfixture専用の空要素によって、用語一行だけが収まり説明は収まらない状態にした。基準版では、`READY`の用語が1ページ目、説明が直後の2ページ目へ分断された。保護版では、`READY`の用語と説明がともに基準版の説明と同じ2ページ目へ移動し、後続の`DONE`も用語と説明が同じページに配置された。
...
- 用語ラベルの空値と許可ノード以外に関する、候補記法の配置、属性または内容モデルの意味検証と診断
- 空の定義リストまたは空の説明
- 用語内の強調、raw HTMLまたはdirectiveを個別に使用した回帰検証
