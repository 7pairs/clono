...
[コラムの調査](vivliostyle-column.md)では、第1章に本文、コラム、本文の順で脚注を配置した。VFMは参照順に`1`、`2`、`3`を付け、コラムから参照した脚注本文をコラム用コンテナの外へ生成した。Vivliostyleは三つの脚注本文を同じ参照ページの下部へ配置した。

第2章では、本文の脚注が`1`、コラム内の脚注が`2`となった。一章を一つのMarkdownファイルとして別々の`entry`へ登録する構成では、本文とコラムの脚注が同じ連番を共有し、章ごとに1へ戻る。

PDFでは、五つの脚注参照と五つの戻りリンクがそれぞれ対応する対象へ解決された。コラム内脚注のインラインコードと外部リンクも保持された。

## 責務判断

コラム内脚注を含むThunder Clawの要件は、現在のVFMとVivliostyleで満たせる。clonoに独自の脚注記法や`span.footnote`への変換を実装しない方針とする。
...
表内の脚注と一つのMarkdownファイルに複数章を含める構成は、具体的な必要性が生じた場合に調査する。

## 再現方法

検証に使用する入力と、HTMLおよびPDFの確認手順は、[基本脚注の検証用fixture](fixtures/vivliostyle-footnotes/)と[コラムとの結合検証用fixture](fixtures/vivliostyle-column/)へ保存している。

基本脚注のfixtureでは、`npm ci`を実行した後、HTMLは`npm run verify:html`、PDFは`npm run build:pdf`で再生成できる。コラムとの結合fixtureでは、`npm ci`を実行した後、`npm run verify`でWeb出版物とPDFを再生成して自動検証できる。PDFの組版結果は各fixtureのREADMEに記載した観点で目視確認する。

## 再調査する条件

- VFMのデフォルト脚注モードまたは`dpub`モードの出力契約が変更された場合
- Vivliostyle.jsによるDPUB-ARIA脚注の組版方法が変更された場合
- コラムの最終的なHTML出力契約で、今回確認した脚注配置や連番を維持できなかった場合
