...
- 各参照の解決先が、対象の表と固有のキャプションのあるページで、キャプションの上方200pt以内かつキャプションの横幅内にある
...
表カウンターには`table`という固有の名前を使用する。[結合検証](vivliostyle-reference-integration.md)では、図の`figure`、コードリストの`listing`と同じ文書へ配置しても、三種類のカウンターが独立して動作することを確認した。
...
この構造により、リンクは表全体を表す`figure`への内部リンクとして解決され、参照文字列には`表1.1 実行環境`のように番号とキャプションだけを表示できた。
...
生成したPDFでは、同一ファイル内、別ファイル間、前方参照、後方参照のすべてが、対象の表全体を表す`figure`への内部リンクとして解決された。
...
- 著者向けの参照記法を、表全体を指す`href`、キャプションを指す`data-title-href`、用途別のclassを持つ空の`a`要素へ変換する
...
個別fixtureではタイトル取得先に`data-caption-href`を使用したが、各参照種別を同じテーマで扱う[結合検証](vivliostyle-reference-integration.md)により、`data-title-href`へ統一した。
