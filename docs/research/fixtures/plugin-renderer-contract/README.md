...
└── src/
    ...
    │   ├── transform_config.cljs
    │   └── verify_transform_config.cljs
    └── test/clono/research/
        ├── plugin_renderer_contract_test.cljs
        └── transform_config_test.cljs
```
...
`clono transform`からプラグインを利用する方法として、次のコマンド形式を候補とする。

```text
clono transform <input> --output <output> --project <project>
```

`--project`は任意とする。指定した場合は、起動時のカレントディレクトリを基準に`<project>`を解決し、その直下にある`clono.config.mjs`だけを読み込む。設定の`plugins`にある相対パスは、`clono.config.mjs`があるディレクトリを基準に解決する。

`--project`を省略した場合は設定を読み込まず、組み込みの既定rendererを使用する。入力Markdownの位置や起動時のカレントディレクトリから、祖先にある`clono.config.mjs`を暗黙に探索しない。この規則により、単独のMarkdownを従来どおり変換でき、プラグインを使用するプレビューでは対象プロジェクトを明示できる。

このコマンド形式、オプション名および設定の`plugins`は検証用の候補であり、正式なCLIおよび設定契約ではない。既存の`clono.config.mjs`へ`plugins`を追加する場合の構造と検証規則は、後続の仕様で決定する。
...
`src/test/clono/research/transform_config_test.cljs`は、`--project`を入力および出力から独立して指定でき、省略時にはプロジェクト設定を選択しないことを確認する。

`src/main/clono/research/verify_transform_config.cljs`をreleaseビルドした`target/verify-transform-config.cjs`は、次を確認する。

- 起動ディレクトリとは異なるプロジェクトを、`--project`の相対パスで明示できる
- 空白と`#`を含むプロジェクトパスおよびプラグインパスを安全な`file:` URLとして読み込める
- `<project>/clono.config.mjs`を評価し、設定ファイルを基準に外部rendererを読み込める
- 読み込んだrendererを単一ファイル変換の候補境界へ渡し、その出力を取得できる
- プロジェクト内から実行しても、`--project`を省略した場合は祖先の設定を探索せず既定rendererを使用する
...
- 正式な`clono transform`における`--project`、設定読み込みおよびrenderer差し替えの統合
- `clono.config.mjs`へ追加する正式なプラグイン設定の構造と検証
