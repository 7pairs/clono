...
垂直余白、強制改行、強制改ページ、段落の右寄せ、定義リストについて、VFMとVivliostyleへ委譲できる範囲と、clonoが変換後に出力すべき構造を検証する。
...
- 空の要素へCSSの`1lh`を指定し、一行相当の垂直余白を表示できる
- 複数の段落を一つのコンテナへ入れ、まとめて右寄せできる
- 用語一つと説明一つを持つ定義リストを表示できる
- 定義の説明内でインラインコード、強調、リンクを使用できる
- 目に見える区切りを表示せず、後続の見出しまたは通常段落を次ページから開始できる

連続する垂直余白、ページ境界にある垂直余白、複数の用語または説明、複数段落を含む定義は対象に含めない。

## 入力と出力の境界

強制改行には標準Markdownのバックスラッシュによる記法を使用する。clonoによる変換は想定しない。

垂直余白、右寄せ、定義リスト、強制改ページには、clonoの変換後を模した次のclassを使用する。

- `clono-space`: CSSの`block-size: 1lh`で一行相当の余白を確保する空の要素
...
- 強制改行が`br`へ変換される
- 垂直余白、右寄せ、定義リスト、強制改ページのID、class、要素構造が保持される
...
- 垂直余白を挟んだ段落間に、本文の行送りとおおむね同じ一行分の余白が追加される
...
- 強制改行、垂直余白、右寄せ、定義リストが意図した見た目である
- 改ページ用の線や記号が表示されない
