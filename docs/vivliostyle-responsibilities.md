...
| 見出し、画像、表、コードリストへの参照用IDの付与 | 必須 | 見出しはVFMの明示的なIDに対応できる。画像と表はIDを持つ`figure`構造をVFMが保持できる。コードリストは未調査 | 見出しIDはVFMの記法を保持する。番号付き画像と表はIDを持つ`figure`へ変換する。未定義・重複IDの診断を担う候補とする。コードリストは未決定 | 調査中 | [見出し](research/vivliostyle-heading-references.md)、[画像](research/vivliostyle-figure-references.md)、[表](research/vivliostyle-table-references.md) |
