...
default exportが存在しない場合、オブジェクトでない場合、必須項目の欠落・型の不一致、対応外の`apiVersion`、または未知の項目・renderer名がある場合は受理しない。`name`の一意性とrendererの競合、失敗時の診断契約は後続の節で定める。`column`関数の入力は次節で定め、戻り値は後続の作業で定める。
