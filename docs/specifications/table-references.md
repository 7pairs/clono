...
2026年9月8日現在、この仕様は実装中である。現在のPull Requestでは、番号付き表の解析、検証、変換、参照対象情報の生成、共通名前空間への統合、および表番号とキャプション番号を表示する基盤CSSを実装する。表参照の解決、表参照用の基盤CSSおよびCLI統合テストは、後続のPull Requestで実装する。
