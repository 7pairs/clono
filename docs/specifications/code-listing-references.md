...
2026年9月11日現在、この仕様は実装中である。最初の実装段階として、番号付きコードリストの著者向け記法、変換、参照対象情報、共通名前空間への統合、リスト番号とキャプションに必要な基盤CSS、および自動テストへ着手する。コードリスト参照の解決、単一ファイル変換用プレースホルダー、参照用の基盤CSS、およびCLI統合テストは後続の実装段階で扱う。
