...
2026年9月11日現在、この仕様は実装済みである。番号付きコードリストの著者向け記法、変換、参照対象情報、共通名前空間への統合、同一原稿および原稿間の参照解決、単一ファイル変換用プレースホルダー、リスト番号、キャプションと参照に必要な基盤CSS、および診断契約を実装している。

単体テストと書籍プロジェクトのテストに加え、releaseビルドしたCLIを使用する統合テストで、単一ファイル変換、書籍全体の参照解決、基盤CSSの配置、および未定義参照時に既存の生成済み原稿ツリーを変更しないことを検証している。
