...
| 12 | 掲載Markdownにおける索引指定の収集と索引Markdownの生成 | 実装済み |
| 13 | ローカルプラグインの読み込みとコラムrendererの書籍変換への適用 | 実装中 |
...
`sourceRoot`、`outputRoot`および`publication`には既定値を設けず、明示必須とする。不足、型の不一致または未知の設定項目がある場合は設定エラーとする。`publication`の各要素についても、この仕様で定義していない項目はエラーとする。プラグイン拡張で追加する省略可能な`plugins`の契約は後述する。

### プラグイン設定（実装中）

実装段階13では、`clono.config.mjs`のdefault exportに省略可能な`plugins`配列を追加する。利用者が信頼するローカルの`.mjs`を、設定ファイルのあるプロジェクトルートからの相対パスで明示する。省略または空配列の場合は組み込みの既定コラムrendererを使用する。パス、プラグインモジュール、競合および診断の正確な契約は[プラグイン仕様](plugins.md)に従う。

実装後は、既存の設定オブジェクトへ`plugins: ["./plugins/column-theme.mjs"]`を追加して指定する。

現行実装は`plugins`の設定値とローカルファイルを検証し、正規化した設定として読み込む。検証済みのES Moduleを設定順に評価し、default export、基本情報および`column` renderer登録を検証する独立したローダーも実装済みである。検証済みrendererのregistry構築と、プラグイン名・renderer名の競合検査も実装済みである。読み込みからregistry構築までを接続する境界では、読み込み・検証に失敗した場合に登録へ進まず、読み込み・検証・競合の診断を共通の結果形式で返す。これらの処理の`clono build`への統合と、rendererの変換への適用はまだ実装していないため、上の設定による出力の差し替えは利用できない。プラグインは変換時に実行するコードであり、プラグイン指定を理由に生成済み原稿ツリーへ追加しない。ただし、プラグインファイルを`sourceRoot`内に置いた場合は、従来の通常ファイルのコピー規則に従って出力にも含まれる。コピーを避けるには`sourceRoot`の外へ配置する。利用者テーマは引き続き`vivliostyle.config.mjs`で管理し、プラグイン設定をテーマ設定の代わりにはしない。
...
実装段階13では、掲載有無にかかわらず、変換計画に含まれるすべての入力Markdownの`column`へ、設定済みのrendererを適用する。通常ファイルのコピー、通過HTML、clonoが生成する空白ページ資材と索引Markdownはプラグインの対象外とする。設定・プラグインの検査は原稿のAST変換前に完了する。プラグインファイル自体のコピーには前述の`sourceRoot`の配置規則が適用される。詳細は[プラグイン仕様](plugins.md)に従う。この拡張は現行実装では利用できない。
...
後述する参照解決の事前検査に成功して文書単位の変換を開始した後に診断が返された場合も、残りのMarkdown変換操作を可能な範囲で継続する。診断は原稿間では変換計画の順、同じ原稿内では入力位置の順に収集する。原稿の読み取りまたは変換処理自体に失敗した場合も、その原稿を示す診断として収集する。
...
掲載Markdownを個別に変換する前に各原稿のASTを解析し、参照対象の収集、書籍全体での重複検査、および参照先の解決を行う。前方参照と後方参照を許可し、参照元と参照先の変換後HTMLパスから、同一文書または別文書を指す相対リンクを生成する。
...
`clono build`では、`publication`で`type`が`document`であり、拡張子を小文字化した結果が`.md`となる原稿だけを索引収集の対象とする。掲載されていないMarkdownは単一文書として索引マーカーへ変換できるが、その索引指定を書籍全体の収集結果または生成索引へ含めない。
...
実装段階13では、プラグイン設定・読み込み・競合またはrendererの実行に失敗した場合も終了コード`1`とする。設定や読み込みの診断には架空の原稿位置を付けず、renderer実行時の診断には該当する`column`の入力位置を付ける。独立した残りの原稿の変換と診断収集は可能な範囲で継続するが、正式な`outputRoot`は変更しない。詳細な診断契約は[プラグイン仕様](plugins.md)を正本とする。
