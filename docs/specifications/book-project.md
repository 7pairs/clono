...
| 5 | staging、backupおよび排他ロックによる安全な出力の公開 | 実装中 |
...
変換計画に含まれるMarkdown変換操作は、計画へ記録された順にすべて処理する。各原稿は独立して単一文書用の変換パイプラインへ渡し、正常に変換できた場合は、変換後のMarkdownを対応する操作へ保持する。この段階では、変換後のMarkdownを正式な`outputRoot`へ書き込まない。

複数の原稿から診断が返された場合も、残りのMarkdown変換操作を可能な範囲で継続する。診断は原稿間では変換計画の順、同じ原稿内では入力位置の順に収集する。原稿の読み取りまたは変換処理自体に失敗した場合も、その原稿を示す診断として収集する。
...
生成処理には、存在しないパスまたは空のディレクトリをstagingとして渡す。変換計画の順にディレクトリ、変換済みMarkdownおよび通常ファイルを生成し、clono基盤CSSをコピーした後、すべての`publication`要素に対応する通常ファイルが存在することを確認する。所有マーカーは、これらの生成と確認がすべて成功した場合にだけ最後に書き込む。
...
すべての処理が成功した場合だけ生成済み原稿ツリーを公開し、終了コード`0`で終了する。標準出力と収集済み診断の表示順序を含む詳細なCLI表示契約は、`build`サブコマンドの実装前に決定する。
