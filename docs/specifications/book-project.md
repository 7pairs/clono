...
現行実装は`plugins`の設定値とローカルファイルを検証し、正規化した設定として読み込む。プラグインモジュールの評価とrendererの適用はまだ実装していないため、上の設定による出力の差し替えは利用できない。プラグインは変換時に実行するコードであり、プラグイン指定を理由に生成済み原稿ツリーへ追加しない。ただし、プラグインファイルを`sourceRoot`内に置いた場合は、従来の通常ファイルのコピー規則に従って出力にも含まれる。コピーを避けるには`sourceRoot`の外へ配置する。利用者テーマは引き続き`vivliostyle.config.mjs`で管理し、プラグイン設定をテーマ設定の代わりにはしない。
