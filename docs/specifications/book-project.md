...
| 13 | ローカルプラグインの読み込みとコラムrendererの書籍変換への適用 | 実装予定 |
...
現行実装は`plugins`を未知の設定項目として拒否するため、上の設定はまだ使用できない。プラグインは変換時に実行するコードであり、プラグイン指定を理由に生成済み原稿ツリーへ追加しない。ただし、プラグインファイルを`sourceRoot`内に置いた場合は、従来の通常ファイルのコピー規則に従って出力にも含まれる。コピーを避けるには`sourceRoot`の外へ配置する。利用者テーマは引き続き`vivliostyle.config.mjs`で管理し、プラグイン設定をテーマ設定の代わりにはしない。
