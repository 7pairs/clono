...
`clono build`では、clonoが管理する参照対象の種類をまたいで、論理IDを書籍全体で一意とする。見出しと番号付き表も同じ名前空間を使用する。実装予定の番号付きコードリストも、[番号付きコードリストとコードリスト参照仕様](code-listing-references.md)に従ってこの名前空間へ統合する。
