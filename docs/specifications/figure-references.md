...
2026年9月4日現在、番号付き画像の変換、`clono transform`における同一原稿内の画像参照の解決と未解決参照のプレースホルダー、および図番号と参照文字列に必要なclono基盤CSSを実装済みである。

`clono build`では、掲載Markdown全体からの参照対象の収集、書籍全体での論理IDと生成HTML IDの重複検査、同一原稿および原稿間の画像参照の解決、ならびに未定義参照の診断を実装済みである。解析、収集、検証または参照解決で診断が発生した場合は原稿を変換せず、既存の生成済み原稿ツリーを変更しない。
...
`clono build`では、clonoが管理する参照対象の種類をまたいで、論理IDを書籍全体で一意とする。見出し参照も同じ名前空間を使用する。将来、表またはコードリストを追加する場合も、この名前空間へ登録する。
...
`xref`は参照種別で共有する。この仕様では`type="figure"`の契約を定め、`type="heading"`の契約は[見出し参照仕様](heading-references.md)で定める。表およびコードリストの参照種別は、それぞれの個別仕様を決定するまで許可しない。

### 番号を持たない対象

前付または後付にある見出しなど番号を持たない参照対象でも、同じ`xref`を使用する。番号を持たない対象では`format="title"`だけを許可し、`number`または`number-title`は表示できる番号が存在しないためエラーとする。番号を持たない対象への参照を、空の出力または表示されないリンクへ変換しない。見出しに対する具体的な規則は[見出し参照仕様](heading-references.md)で定める。
...
- 新しい参照種別へ`xref`を拡張する場合は、対応する個別仕様を追加し、この文書、[見出し参照仕様](heading-references.md)および[clono著者向け記法](authoring-syntax.md)の共通契約を同時に見直す
