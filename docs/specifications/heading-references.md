...
2026年9月6日現在、初期仕様の対象となる見出しの収集、IDの検証、`xref`の`heading`種別、同一原稿および原稿間の参照解決、単一ファイル変換用のプレースホルダー、診断、ならびに見出し参照用のclono基盤CSSを実装済みである。

単一ファイル変換、書籍プロジェクト変換、およびreleaseビルドしたCLIを通した自動テストで、見出し参照の変換と失敗時の出力契約を検証している。
...
`clono build`では、clonoが管理する見出し、番号付き画像および番号付き表の論理IDを書籍全体で一意とする。実装予定の番号付きコードリストも、[番号付きコードリストとコードリスト参照仕様](code-listing-references.md)に従ってこの名前空間へ統合する。また、論理IDが異なる場合も、次のように見出しのHTML IDと他の参照対象から導出したHTML IDが衝突する場合はエラーとする。
